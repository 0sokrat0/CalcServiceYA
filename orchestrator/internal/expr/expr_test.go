...
func TestCreateExp(t *testing.T) {
	exprStore := db.NewExpressionStore()
	...
	exprRecord, err := CreateExp(exprStore, taskStore, expression, id, cfg)
	assert.NoError(t, err)

	rec, found := exprStore.GetByID(id)
	assert.True(t, found)

	assert.Equal(t, exprRecord, *rec)
